...
private:
    ...
    {
			for (unsigned int i = 0; ptrLookAt->childrenPtrs.size(); i++)
			{
				if (ptrLookAt->childrenPtrs[i]->isDirectory)
				{
					return(Find(filename, File* (ptrLookAt->childrenPtrs[i])));
				}
				
				if (ptrLookAt->childrenPtrs[i]->name == filename)
				{
					return (ptrLookAt->childrenPtrs[i]);
    ...
    {
        /*for (unsigned int i=0;current->childrenPtrs.size(); i++)
		{
			if (current->childrenPtrs[i]->name == folder you want)
			{
				//...
			}
		}
