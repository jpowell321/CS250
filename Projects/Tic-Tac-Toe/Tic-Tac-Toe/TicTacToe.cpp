...
//	cout << m_gameHistory.size() << " moves" << endl;
	cout << "Stack 2 - " << m_gameHistory2.GetStackSize() << " moves" << endl;
	m_board.Display();
}
...
//	cout << "Undo last move, stack size: " << m_gameHistory.size() << endl;
	cout << "Stack 2 - Undo last move, stack size: " << m_gameHistory2.GetStackSize() << endl;
...
*/
	if (m_gameHistory2.GetStackSize() >= 1)
	{
		// The top-most item is the current state,
		// so actually need to pop this and then get top.
        m_gameHistory2.pop();
	    m_board = m_gameHistory2.top();
      	m_turn--;
	}
	m_board.Display();
}
